/**
 * LoginAndroid - Android-specific login component
 * Handles fingerprint and face unlock with Android-optimized UX
 */

import { StyleSheet, Text, View, TouchableOpacity, TextInput, BackHandler, Alert, Platform } from 'react-native';
...
export default function LoginAndroid() {
  ...
  const tutorialSteps = [
    {
      title: 'Welcome to Android Login',
      description: 'Enter your registered phone number to access your account securely on Android.'
    },
    {
      title: 'Android Biometric Features',
      description: 'On Android, you can use fingerprint or face unlock for quick access after initial setup.'
    },
    {
      title: 'Phone Number Format',
      description: 'Please enter your phone number in the correct format (e.g., 09XXXXXXXXX).'
    }
  ];
  ...
  useEffect(() => {
    const checkFirstTimeUser = async () => {
      try {
        const isFirstTime = await AsyncStorage.getItem('login_tutorial_shown_android');
        if (!isFirstTime) {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      const result = await biometricManager.authenticate({
        promptMessage: 'Use your fingerprint or face to sign in',
  ...
  const renderBiometricButton = () => {
    if (!biometricButtonVisible || !canUseBiometric) return null;

    const primaryType = biometricManager.getPrimaryBiometricType();
    const isFingerprint = primaryType?.type === 'FINGERPRINT';
    const iconName = isFingerprint ? 'fingerprint' : 'face-recognition';
    const buttonText = isFingerprint ? 'Use Fingerprint' : 'Use Face Unlock';

    return (
      <TouchableOpacity 
        style={[loginstyle.biometricButton, isLoading && { opacity: 0.5 }]}
  ...
  return (
    <View style={loginstyle.container}>
      <TutorialModal
        ...
        steps={tutorialSteps}
        storageKey="login_tutorial_shown_android"
